...
var (
	DefaultPlatform = "zac"         // default platform name used for login
	DefaultVersion  = "18.0.4-4.26" // platform version
)

// Login describes the information for authorization on the MX server.
type Login struct {
	Type     string `yaml:",omitempty" json:"type,omitempty"`     // account type: User, Server, Group
	User     string `json:"user,omitempty"`                       // username
	Password string `json:"password,omitempty"`                   // password
	Clear    bool   `yaml:",omitempty" json:"clear,omitempty"`    // send password in clear text
	Platform string `yaml:",omitempty" json:"platform,omitempty"` // platform name
	Version  string `yaml:",omitempty" json:"version,omitempty"`  // software version
}
